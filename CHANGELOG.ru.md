...
- Клиент Ynison для реального времени (`YandexMusic.Ynison`): подписка на состояние воспроизведения
  ...
  трек, громкость отдельного устройства) по тому же websocket-протоколу, что используют официальные
  клиенты. Полные модели protobuf-JSON состояния, переподключение с ограниченным экспоненциальным
  бэкоффом, готовые билдеры запросов и `YandexMusicClient.CreateYnisonClient()`, автоматически
  подставляющий токен сессии.
